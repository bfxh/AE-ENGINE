            ...
            for j in 0..self.config.ny {
                for i in 0..self.config.nx {
                    ...
                    if d.length_squared() < r2 {
                        let lin_idx = self.idx(i, j, k);
                        self.beta_field[lin_idx] = beta;
                    }
                }
            }
        }
        ...
        for k in 1..nz - 1 {
            for j in 1..ny - 1 {
                for i in 1..nx - 1 {
                    ...
                    // 离散 Laplacian: ∇²p
                    let lap = (p_xp + p_xm + p_yp + p_ym + p_zp + p_zm - 6.0 * p) / (h * h);

                    // 标准 FDTD: p^{n+1} = 2 p^n - p^{n-1} + (c·dt)² · ∇²p
                    let mut p_new = 2.0 * p - p_prev + c2_dt2_over_h2 * (lap * h * h);
                    // 注意: c2_dt2_over_h2 已包含 h²，所以这里直接用 (邻居和 - 6p)
                    p_new = 2.0 * p - p_prev + c2_dt2_over_h2 * (p_xp + p_xm + p_yp + p_ym + p_zp + p_zm - 6.0 * p);
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // 时间混合
    let beta = mix(beta_field[idx], params.global_beta, step(0.0, params.global_beta));
    if beta > 0.0 {
        p_new = (1.0 - beta) * p_new + beta * p;
    }
    ...
    for (s_idx, s) in enumerate(sources)) {
        if u32(s_idx) >= params.source_count {
            break;
        }
        let _ = s;
...
pub mod gpu {
    ...
    impl GpuWaveBlenderDispatcher {
        pub async fn new(config: WaveBlenderConfig) -> Self {
            ...
            let source_buffer = make_storage("sources", 64 * std::mem::size_of::<SourceGpu>() as ugpu_size());
