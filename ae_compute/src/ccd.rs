...
pub fn gjk_distance<A: Collider, B: Collider>(a: &A, b: &B) -> DistanceResult {
    ...
    // 从 MD 点反推 A 和 B 上的最近点:
    // MD 点 = support_a(d) - support_b(-d), 其中 d = closest 方向
    // 但 GJK 收敛时 simplex 顶点已是 support 点, 取最后一个 simplex 中
    // 离原点最近的顶点对应的 (a, b) 对. 这里用 support 重算更稳妥.
    let closest_a = a.support(normal);
    let closest_b = b.support(-normal);
    // 修正: closest = a_pt - b_pt, 所以 closest_a 应沿 +normal, closest_b 沿 -normal
    // 但 support(normal) 返回 A 上沿 normal 最远点 (即离 B 最近的方向)
    // 反推: a_pt - b_pt = closest, a_pt = b_pt + closest
    // 简化: 取 a.support(normal) 为 A 上离 B 最近点, b.support(-normal) 为 B 上离 A 最近点
    DistanceResult {
        distance: dist,
        closest_a,
        closest_b,
...
fn do_simplex_triangle(simplex: &[Vec3]) -> (Vec<Vec3>, Vec3, Vec3, bool) {
    ...
    // 原点在三角形内部 (面区域)
    // 检查是否包含原点 (3D 中三角形不可能包含原点, 除非原点在三角形平面上)
    let dist_to_plane = abc.dot(ao) / abc.length();
    if dist_to_plane.abs() < GJK_DISTANCE_TOLERANCE {
        // 原点在三角形平面上: 在面内, 返回原点投影
        let n = abc / abc.length();
        // 投影原点到三角形
        let t = -a.dot(n) / 1.0; // n 已归一化, dist = a·n
        let closest = Vec3::ZERO + n * (-t); // 等价于原点减去法线方向的距离
        // 简化: 原点在三角形上, 最近点就是原点本身 (0)
        return (vec![c, b, a], Vec3::ZERO, Vec3::ZERO, true);
    }
    // 否则原点在三角形正/负侧, 用法线方向继续搜索
    let dir = if abc.dot(ao) > 0.0 { abc } else { -abc };
    (vec![c, b, a], dir, abc * (abc.dot(ao) / abc.length_squared()), false)
}
...
/// 通用 Conservative Advancement: 求两运动凸形状的最早接触时刻
///
/// 输入:
/// - a, b: 起始时刻的形状 (任意 Collider)
/// - motion_a, motion_b: A 和 B 在 [0, dt] 内的运动
/// - dt: 时间区间长度
/// 输出: 最早 TOI (None 表示在 [0, dt] 内无接触)
///
/// 算法:
/// 1. 在当前时刻 t 用 gjk_distance 查询两形状距离 d
/// 2. 若 d ≤ tol -> 已接触, 返回 t
/// 3. 估算相对速度上界 v_max = motion_a.speed_bound() + motion_b.speed_bound()
/// 4. 安全步长 δt = d / v_max (此时 d 在 δt 内不会降到 0)
/// 5. 推进 t += δt, 重新计算形状位置, 重复
///
/// 旋转处理: 用 max_radius 估算角速度贡献的线速度上界
pub fn conservative_advancement<A: Collider, B: Collider>(
    a_start: &A,
    b_start: &B,
    motion_a: &Motion,
    motion_b: &Motion,
    ...
    let mut t = 0.0f32;
    let v_rel_bound = motion_a.speed_bound() + motion_b.speed_bound();
    if v_rel_bound < 1e-12 {
        return None; // 两者都静止, 不可能新接触
    }

    for _ in 0..CA_MAX_ITER {
        // 当前时刻的形状 (按 t 推进)
        let (a_now, b_now) = advance_colliders(a_start, b_start, motion_a, motion_b, t);
        let result = gjk_distance(&a_now, &b_now);

        if result.intersecting {
            return Some(Toi {
                t,
                normal: result.normal,
                point: (result.closest_a + result.closest_b) * 0.5,
            });
        }
        if result.distance <= CA_DISTANCE_TOLERANCE {
            return Some(Toi {
                t,
                normal: result.normal,
                point: (result.closest_a + result.closest_b) * 0.5,
        ...
        // 安全步长
        let dt_step = result.distance / v_rel_bound;
        let new_t = t + dt_step;
        if new_t >= dt {
            // 推进到区间末尾仍未接触
            let (a_end, b_end) = advance_colliders(a_start, b_start, motion_a, motion_b, dt);
            let end_result = gjk_distance(&a_end, &b_end);
            if end_result.intersecting || end_result.distance <= CA_DISTANCE_TOLERANCE {
                return Some(Toi {
                    t: dt,
                    normal: end_result.normal,
                    point: (end_result.closest_a + end_result.closest_b) * 0.5,
                });
            }
            return None;
        }
        t = new_t;
        if dt - t < CA_TIME_TOLERANCE {
            // 已接近区间末尾
            let (a_end, b_end) = advance_colliders(a_start, b_start, motion_a, motion_b, t);
            let end_result = gjk_distance(&a_end, &b_end);
            if end_result.intersecting || end_result.distance <= CA_DISTANCE_TOLERANCE {
                return Some(Toi {
                    t,
...
/// 推进 Collider 到时刻 t. 由于 Collider 不可变, 用包装类型实现.
/// 简化策略: 用一个 SphereCollider / BoxCollider 的本地副本通过 closure.
/// 但 generic 上不易实现, 故提供具体形状的 advance_ 函数.
fn advance_colliders<A: Collider, B: Collider>(
    _a_start: &A,
    _b_start: &B,
    _motion_a: &Motion,
    _motion_b: &Motion,
...
where
    ...
    B: Clone,
{
    // 此通用版本仅在 A, B: Clone 时可用. 实际 CA 调用方应使用具体类型版本.
    unimplemented!("use conservative_advancement_sphere_box 等具体类型版本")
}
...
/// 球-球 CA (其实有解析解, 这里也提供 CA 版本作为对比和兜底)
pub fn conservative_advancement_sphere_sphere(
    ...
    for _ in 0..CA_MAX_ITER {
        let a_now = advance_sphere(a_start, motion_a, t);
        let b_now = advance_sphere(b_start, motion_b, t);
        let result = gjk_distance(&a_now, &b_now);
        if result.intersecting || result.distance <= CA_DISTANCE_TOLERANCE {
            return Some(Toi {
                t,
                normal: result.normal,
                point: (result.closest_a + result.closest_b) * 0.5,
...
pub fn conservative_advancement_sphere_box(
    ...
    let mut t = 0.0f32;
    let v_rel_bound = motion_sphere.speed_bound() + motion_box.speed_bound();
    if v_rel_bound < 1e-12 {
        return None;
    }
    for _ in 0..CA_MAX_ITER {
        let s_now = advance_sphere(sphere_start, motion_sphere, t);
        let b_now = advance_box(box_start, motion_box, t);
        let result = gjk_distance(&s_now, &b_now);
        if result.intersecting || result.distance <= CA_DISTANCE_TOLERANCE {
            return Some(Toi {
                t,
                normal: result.normal,
        ...
        if new_t >= dt {
            // 末尾检查
            let s_end = advance_sphere(sphere_start, motion_sphere, dt);
            let b_end = advance_box(box_start, motion_box, dt);
            let end_result = gjk_distance(&s_end, &b_end);
            if end_result.intersecting || end_result.distance <= CA_DISTANCE_TOLERANCE {
                return Some(Toi {
                    t: dt,
                    normal: end_result.normal,
...
pub fn swept_sphere_vs_sphere(
    ...
    let normal = (point - target_center).normalize_or_zero();
