...
//!
//! 优点:
//! - XPBD 比 PBD 有明确刚度 (无 iterations 依赖)
//! - 无条件稳定 (大时间步)
//! - 自然处理大变形 (折叠、皱纹)
//! - 弯曲用距离约束近似 (稳定, 无需 dihedral angle 解析梯度)
...
impl ClothMesh {
    /// 创建平面布料 (n_u x n_v 网格)
    /// origin: 左上角; u_dir: 水平方向总向量; v_dir: 垂直方向总向量
    pub fn plane(
        origin: Vec3,
        u_dir: Vec3,
        v_dir: Vec3,
        ...
        // 水平: 每行中 (i-1, j) -- (i+1, j), rest = 2*seg_u
        ...
        // 垂直: 每列中 (i, j-1) -- (i, j+1), rest = 2*seg_v
    ...
    /// 设置 stiffness (per-constraint multiplier)
    ...
    fn solve_distance_constraints(&mut self, kind: ConstraintKind) {
        ...
        for mesh in &mut self.meshes {
            ...
            for (ci, &(a, b, rest_len, k)) in constraints.iter().enumerate() {
                ...
                // XPBD: α̃ = α/(h²·k), Δλ = (-C - α̃·λ_old) / (w_sum + α̃)
                ...
                // C>0 (拉伸) → Δλ<0 → a 朝 +dir (朝 b), b 朝 -dir (朝 a) → 距离缩小 ✓
                let dir = diff / dist;
