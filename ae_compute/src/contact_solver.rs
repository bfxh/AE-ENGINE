...
fn integrate_rotation(q: glam::Quat, delta_rot: Vec3) -> glam::Quat {
    ...
    let result = glam::Quat::from_xyzw(
        q.x + dq.x,
        q.y + dq.y,
        q.z + dq.z,
        q.w + dq.w,
    ...
    pub fn solve_velocity(&mut self, bodies: &mut [RigidBody], dt: f32) {
        for _ in 0..self.velocity_iterations {
            for m in &mut self.manifolds {
                ...
                solve_manifold_velocity(m, a, b, friction, dt);
            }
        }
    }
...
fn solve_manifold_velocity(
    ...
    for cp in &mut m.points {
        ...
        // λ_n = -(vn + e·vn_init + bias) / k_n
        // 简化: 用当前 vn (已含累积效应)
        let lambda_n = -(vn + restitution * vn + position_bias) / k_n;
        // 累积, 并 clamp ≥ 0 (法向约束是单侧的: 只推不拉)
...
mod tests {
    ...
    fn test_restitution_bounce() {
        ...
        let mut bodies = vec![
            make_static_floor(),
            make_box_at(1.0, Vec3::new(0.0, 0.9, 0.0), Vec3::splat(1.0)),
        ];
    ...
    fn test_friction_slows_sliding() {
        ...
        let mut bodies = vec![
            make_static_floor(),
            make_box_at(1.0, Vec3::new(0.0, 0.0, 0.0), Vec3::splat(1.0)),
        ];
    ...
    fn test_warm_start_preserves_impulse() {
        ...
        assert!(
            impulse_frame1 > 0.0,
            "should have accumulated impulse, got {}",
            impulse_frame1
        );
    }
