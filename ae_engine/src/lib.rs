...
use ae_biology::prelude::*;
use ae_chemistry::reactions::HazardType;
use ae_metaentity::prelude::*;
use ae_physics::prelude::*;
use ae_thermo::prelude::*;

// fixed_point types re-exported below via pub use

use ae_metaentity::interaction_cache::CacheStats as MetaCacheStats;
use ae_metaentity::meta_entity::CellType as MetaCellType;
use ae_metaentity::meta_entity::Element as MetaElement;

use ae_acoustics::prelude::*;
use ae_axiom::prelude::*;
use ae_electro::prelude::*;
use ae_factory::prelude::*;
use ae_geo::prelude::*;
use ae_optics::prelude::*;

pub use ae_biology::ecosystem::Biome;
pub use ae_chemistry::reactions::{
    ChemicalReaction, HazardType as ChemHazard, ReactionType,
};
pub use ae_physics::dual_phase::{DualPhaseEntity, DualPhaseManager, PhasePriority};
pub use ae_physics::fixed_point::{FixedPoint, FixedQuat, FixedVec3};
pub use ae_physics::material::MaterialCategory;
pub use ae_physics::material::MaterialProperties;
pub use ae_physics::octree::SparseOctree;
pub use ae_physics::world::{BodyType, RigidBody};
...
// 游戏逻辑层重导出（实际实现位于 ae_game crate）
pub use ae_game::{building::*, combat::*, economy::*, infection::*, npc::*};
...
impl GameWorld {
    ...
    pub fn tick(&mut self) {
        ...
        let rigid_sync: Vec<(usize, [f32; 3], [f32; 3])> = {
            ...
            sim.physics
                .rigid_bodies
                .iter()
                .filter(|b| b.body_type != ae_physics::world::BodyType::Static)
                .filter_map(|b| {
    ...
    pub fn spawn_meta_entity(&mut self, mut entity: MetaEntity) -> Uuid {
        ...
        let material_idx = if entity
            ...
            .elemental_composition
            .iter()
            .any(|ef| matches!(ef.element, ae_metaentity::meta_entity::Element::Fe))
        {
            3
    ...
    pub fn build_structural_field(&mut self, entity_ids: &[Uuid]) {
        let entities: Vec<(
            ...
            Vec3,
            f32,
            Vec<ae_metaentity::structural_field::ConstraintEdge>,
        )> = entity_ids
            .iter()
            ...
            for y in 0..grid.resolution.y {
                for x in 0..grid.resolution.x {
                    ...
                    if let Some(voxel) = grid.get_voxel(pos) {
                        if !voxel.flags.contains(ae_physics::destruction::VoxelFlags::ACTIVE)
                        {
                            continue;
                        }
                        ...
                        let color = if voxel
                            .flags
                            .contains(ae_physics::destruction::VoxelFlags::ON_FIRE)
                        {
                            [1.0, 0.3 + temp_ratio * 0.5, 0.0, 1.0]
                        } else if voxel
                            .flags
                            .contains(ae_physics::destruction::VoxelFlags::CORRODED)
                        {
                            [0.6 + stain * 0.4, 0.3 + stain * 0.2, 0.1 + stain * 0.1, 1.0]
                        } else if voxel
                            .flags
                            .contains(ae_physics::destruction::VoxelFlags::IRRADIATED)
                        {
                            [0.2, 0.8, 0.2, 1.0]
                        } else {
                            let base = match grid.material.category {
                                ae_physics::material::MaterialCategory::Concrete => {
                                    [0.5, 0.48, 0.45]
                                },
                                ae_physics::material::MaterialCategory::Metal => {
                                    [0.4, 0.35, 0.3]
                                },
                                ae_physics::material::MaterialCategory::Stone => {
                                    [0.55, 0.5, 0.45]
                                },
                                ae_physics::material::MaterialCategory::Wood => {
                                    [0.4, 0.25, 0.1]
                                },
                                _ => [0.5, 0.5, 0.5],
                            };
        ...
        self.game_logic.ecosystems[ecosystem_index]
            ...
            .map(|f| {
                let species_id = match f.species {
                    ae_biology::ecosystem::FloraSpecies::DeadTree => 0u32,
                    ae_biology::ecosystem::FloraSpecies::MutatedGrass => 1,
                    ae_biology::ecosystem::FloraSpecies::GlowingMushroom => 2,
                    ae_biology::ecosystem::FloraSpecies::ThornBush => 3,
                    ae_biology::ecosystem::FloraSpecies::MutfruitTree => 4,
                    ae_biology::ecosystem::FloraSpecies::TatoPlant => 5,
                    ae_biology::ecosystem::FloraSpecies::Razorgrain => 6,
                    ae_biology::ecosystem::FloraSpecies::Hubflower => 7,
                    ae_biology::ecosystem::FloraSpecies::Bloodleaf => 8,
                    ae_biology::ecosystem::FloraSpecies::BrainFungus => 9,
                    ae_biology::ecosystem::FloraSpecies::Firecap => 10,
                    ae_biology::ecosystem::FloraSpecies::Tarberry => 11,
                    ae_biology::ecosystem::FloraSpecies::AshBlossom => 12,
                    ae_biology::ecosystem::FloraSpecies::Custom(id) => id,
                };
        ...
        self.game_logic.ecosystems[ecosystem_index]
            ...
            .map(|o| {
                let species_id = match o.species {
                    ae_biology::organisms::Species::Human => 0u32,
                    ae_biology::organisms::Species::MutantHuman => 1,
                    ae_biology::organisms::Species::Ghoul => 2,
                    ae_biology::organisms::Species::SuperMutant => 3,
                    ae_biology::organisms::Species::Radroach => 4,
                    ae_biology::organisms::Species::Molerat => 5,
                    ae_biology::organisms::Species::Deathclaw => 6,
                    ae_biology::organisms::Species::Brahmin => 7,
                    ae_biology::organisms::Species::Bloatfly => 8,
                    ae_biology::organisms::Species::Radscorpion => 9,
                    ae_biology::organisms::Species::YaoGuai => 10,
                    ae_biology::organisms::Species::MutantHound => 11,
                    ae_biology::organisms::Species::GiantAnt => 12,
                    ae_biology::organisms::Species::Cazador => 13,
                    ae_biology::organisms::Species::Gecko => 14,
                    ae_biology::organisms::Species::Mantis => 15,
                    ae_biology::organisms::Species::Custom(id) => id,
                };
                let alive = if o.state == ae_biology::organisms::OrganismState::Dead {
                    0.0f32
                } else {
                    1.0
                };
    ...
    pub fn spawn_iron_particles(
        ...
        if granular {
            self.simulation.particle_system.spawn_granular(
                ae_particle::particles::ElementType::Iron,
                pos,
    ...
    pub fn npc_dialogue(
        ...
        npc_id: Uuid,
        player_message: &str,
    ) -> Option<ae_ai_bridge::character_bridge::DialogueResponse> {
