use glam::Vec3;
use ae_asset_pipeline::{AssetFormat, AssetPipeline, BlueprintImporter};
use ae_engine::{BodyType, FixedPoint, FixedQuat, FixedVec3, MaterialProperties, RigidBody};
use ae_metaentity::meta_entity::{EntityChanges, MetaEntity};
use ae_physics::world::PhysicsWorld;
use ae_registry::ResponseRegistry;
use ae_save_system::{SaveFormat, SaveMetadata, SaveSystem};
use ae_scheduler::{EventType, Scheduler};
use ae_unified_interface::{UnifiedWorld, WorldStorage};
use ae_engine::EngineUuid;
...
fn test_save_load_roundtrip() {
    let save_dir = std::env::temp_dir().join("ae_integration_test");
