...
//!
//! 运行：cargo run --release -p ae_thermo_sandbox --bin thermo_sandbox_multi_demo
...
fn main() {
    ...
    for idx in 0..sb.cells.len() {
        if sb.cells[idx].kind == ae_thermo_sandbox::CellKind::Gas {
        ...
        if sb.cells[idx].kind == ae_thermo_sandbox::CellKind::Wood {
