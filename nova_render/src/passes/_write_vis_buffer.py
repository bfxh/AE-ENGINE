...
content = r'''//! Visibility Buffer Pass: 仅记录可见几何 ID（triangle + instance + barycentric）
//!
...
use crate::passes::forward::MeshInstanceData;
...
impl RenderGraphNode for VisibilityBufferPass {
    ...
    fn execute(&mut self, _ctx: &mut NodeContext) -> NodeResult {
        // TODO: RenderGraph 调度 — 需要 mesh vertex/index buffer + instance data
        // 当前实现完成 pipeline 与资源初始化，draw 由 RenderGraph 调度器调用：
        //   1. encoder.begin_render_pass(...) — color = vis_view, depth = depth_view
        //   2. render_pass.set_pipeline(&pipeline)
        //   3. render_pass.set_bind_group(0, &bind_group, &[])
        //   4. render_pass.set_vertex_buffer(0, vertex_buffer.slice(..))
        //   5. render_pass.set_vertex_buffer(1, instance_buffer.slice(..))
        //   6. render_pass.set_index_buffer(index_buffer, ...)
        //   7. render_pass.draw_indexed(0..num_indices, 0, 0..num_instances)
        Ok(())
    }
}
'''

target = r"d:\rj\wasteland_project\nova_render\src\passes\visibility_buffer.rs"
