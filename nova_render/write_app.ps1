$path = "d:\rj\wasteland_project\nova_render\src\application.rs"
$content = @'
...
///
/// 生命周期：
/// 1. `builder().build()` → 构造 NovaApp（wgpu 字段全为 None）
/// 2. `run()` → 创建 EventLoop，进入 winit 事件循环
/// 3. `resumed` 事件 → 创建 Window + 初始化 wgpu（Instance/Adapter/Device/Queue/Surface）
/// 4. `RedrawRequested` 事件 → acquire surface texture → execute RenderGraph → present
/// 5. `CloseRequested` 事件 → 退出
pub struct NovaApp {
    // ===== winit =====
...
impl NovaApp {
    ...
    fn handle_resize(&mut self, width: u32, height: u32) {
        ...
        if let Some(f) = self.on_resize.as_mut() {
            f(self, width, height);
        }
    }
    ...
    fn handle_redraw(&mut self) {
        ...
        // 3. 创建 texture view（按任务规范；当前 graph 节点未直接消费，预留供后续 pass 使用）
        let _view = frame
            .texture
        ...
        // 5. 执行 RenderGraph
        //    利用 Rust disjoint field borrows：&mut self.graph 与 &self.device / &self.queue 同时存在
        let report = self.graph.execute(
...
$info = Get-Item $path
Write-Output "Written: $($info.FullName)  size=$($info.Length) bytes"
