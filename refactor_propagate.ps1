...
$propagateStart = -1
$propagateEnd = -1
$thermalUpdateStart = -1
for ($i = 0; $i -lt $lines.Length; $i++) {
    if ($lines[$i] -match '^\s*fn propagate_cross_domain_effects') { $propagateStart = $i }
    if ($propagateStart -ge 0 -and $thermalUpdateStart -lt 0 -and $lines[$i] -match '^\s*fn thermal_update') {
        $propagateEnd = $i - 1
        $thermalUpdateStart = $i
    }
}

Write-Output "propagate_cross_domain_effects: lines $($propagateStart+1) to $($propagateEnd+1)"
Write-Output "thermal_update starts at: $($thermalUpdateStart+1)"

# 构建新代码（4个函数替换1个大函数）
$newCode = @(
    '    /// 发布跨域事件到 EventBus（不直接处理，由 process_cross_domain_events 分发）'
    ...
    '    }'
    ''
    '    /// 处理碰撞伤害事件（原 propagate_cross_domain_effects 中的碰撞分支）'
    ...
    '    }'
    ''
    '    /// 处理化学反应事件（原 propagate_cross_domain_effects 中的化学分支）'
    ...
    '                                    (voxel.temperature - FixedPoint::from_f32(350.0)).to_f32() * dt * 0.1,'
    '                                    "thermal",'
    '                                );'
    '                            }'
    '                        }'
    ...
    '    }'
    ''
    '    #[deprecated(note = "使用 publish_cross_domain_events + process_cross_domain_events 替代")]'
    '    fn propagate_cross_domain_effects(&mut self, dt: f32) {'
...
# 替换 propagate_cross_domain_effects 函数体（从 fn propagate 到 fn thermal_update 之前）
$newLines = @()
$newLines += $lines[0..($propagateStart-1)]
$newLines += $newCode
$newLines += $lines[$thermalUpdateStart..($lines.Length-1)]

# 写回文件（UTF-8 无 BOM）
$utf8NoBom = New-Object System.Text.UTF8Encoding $false
[System.IO.File]::WriteAllLines($path, $newLines, $utf8NoBom)
Write-Output "Done. Old lines: $($propagateEnd - $propagateStart + 1), New lines: $($newCode.Count)"
