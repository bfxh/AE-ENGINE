...
AE-ENGINE中所有实体——无论是一块石头、一瓶酸液、还是一个变异生物——都用同一套统一建模语言来描述。这套语言的核心是元体属性向量，它不区分物理、化学、生物，只是把所有属性都列在同一个数据结构里。每个元体都有一组属性，包括物理量（质量、密度、硬度）、化学量（元素组成、反应活性）、生物量（基因Token序列、代谢速率）。属性值可以为零，但字段永远存在。
...
**这就是AE-ENGINE的最终技术方案。数据统一，接口统一，计算用最狠的专属引擎，调度用确定性并行。**