...
- **覆盖率**: 47/47 模块（100%）
- **验证命令**: `cargo test -p wasteland_biology --lib` → ok. 1128 passed; 0 failed
...
cargo check -p wasteland_biology --manifest-path d:\rj\wasteland_project\Cargo.toml --target-dir d:\rj\wasteland_project\target2 --lib --tests
cargo test -p wasteland_biology --manifest-path d:\rj\wasteland_project\Cargo.toml --target-dir d:\rj\wasteland_project\target2 --lib
```
