...
impl DynamicAabbTree {
    ...
    pub fn validate(&self) -> bool {
        ...
        self.validate_subtree(self.root).is_ok()
    }

    /// 验证树结构, 返回错误描述 (调试用)
    pub fn validate_diagnostic(&self) -> Result<(), String> {
        if self.root == NULL_NODE {
            return Ok(());
        }
    ...
    fn validate_subtree(&self, idx: i32) -> Result<(), String> {
        if idx == NULL_NODE {
            return Ok(());
        }
        let node = &self.nodes[idx as usize];
        if node.is_leaf() {
            if node.child1 != NULL_NODE || node.child2 != NULL_NODE {
                return Err(format!("leaf {} has children", idx));
            }
            if node.height != 0 {
                return Err(format!("leaf {} height={} (expected 0)", idx, node.height));
            }
            return Ok(());
        }
        let c1 = node.child1;
        ...
        self.validate_subtree(c1)?;
        self.validate_subtree(c2)?;
        Ok(())
    }
}
...
mod tests {
    ...
    fn test_quality_metric() {
        ...
        assert!(q > 0.5 && q < 30.0, "tree quality: {}", q);
    }
