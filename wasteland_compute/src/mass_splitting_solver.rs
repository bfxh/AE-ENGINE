...
impl MassSplittingSolver {
    ...
    fn effective_mass_split(
        ...
        let na = n_a as f32;
        let nb = n_b as f32;
        // 线性项: 质量分割 — 每个接触分得 m/n 的质量, 即逆质量乘以 n
        // (n 个接触共享该体, 每个接触"看到"的有效逆质量 = n * inv_mass)
    ...
    ///
    /// 累积冲量跟踪: 用本地缓冲 acc_n/acc_t1/acc_t2 (非 cp 字段),
    /// 迭代中更新, 用于摩擦锥 clamp. 迭代结束后写回 cp 供下一帧 warm start.
    pub fn solve_velocity(&mut self, bodies: &mut [RigidBody], dt: f32) {
        ...
        // 累积冲量缓冲 (从 cp 初始化, 迭代中更新, 最后写回 cp)
        let mut acc_n: Vec<Vec<f32>> = Vec::with_capacity(self.manifolds.len());
        let mut acc_t1: Vec<Vec<f32>> = Vec::with_capacity(self.manifolds.len());
        let mut acc_t2: Vec<Vec<f32>> = Vec::with_capacity(self.manifolds.len());

        for m in &self.manifolds {
            let a_idx = m.a_index as usize;
            let b_idx = m.b_index as usize;
            let mut biases = Vec::with_capacity(m.points.len());
            let mut ns = Vec::with_capacity(m.points.len());
            let mut t1s = Vec::with_capacity(m.points.len());
            let mut t2s = Vec::with_capacity(m.points.len());
            if a_idx >= bodies.len() || b_idx >= bodies.len() {
            ...
            for cp in &m.points {
                ...
                ns.push(cp.normal_impulse);
                t1s.push(cp.tangent_impulse1);
                t2s.push(cp.tangent_impulse2);
            }
            all_biases.push(biases);
            acc_n.push(ns);
            acc_t1.push(t1s);
        ...
        for _ in 0..self.velocity_iterations {
            ...
            for (m_idx, m) in self.manifolds.iter().enumerate() {
                ...
                for (i, cp) in m.points.iter().enumerate() {
                    ...
                    // 累积, clamp >= 0 (使用本地缓冲, 可变)
                    let old_n = acc_n[m_idx][i];
                    ...
                    let old_t1 = acc_t1[m_idx][i];
                    let old_t2 = acc_t2[m_idx][i];
                    let new_t1 = old_t1 + lambda_t1;
                    let new_t2 = old_t2 + lambda_t2;

                    // 摩擦锥 clamp (使用累积法向冲量 new_n, 非单次迭代冲量)
                    ...
                    acc_t1[m_idx][i] = scaled_t1;
                    acc_t2[m_idx][i] = scaled_t2;
        ...
        // 写回累积冲量到 cp, 供下一帧 warm start
        for (m_idx, m) in self.manifolds.iter_mut().enumerate() {
            for (i, cp) in m.points.iter_mut().enumerate() {
                cp.normal_impulse = acc_n[m_idx][i];
                cp.tangent_impulse1 = acc_t1[m_idx][i];
                cp.tangent_impulse2 = acc_t2[m_idx][i];
            }
        }
    }
...
mod tests {
    ...
    fn test_effective_mass_split_reduces_value() {
        ...
        // n_a = n_b = 4 (分割: 每个接触分得 1/4 质量, 逆质量 *4, k 变大)
    ...
    fn test_high_friction_stops_sliding() {
        // 单接触点 + 高摩擦: 滑动转化为滚动 (角动量守恒 around 接触点).
        // CoM 速度不会归零, 但接触点切向速度应趋近 0 (纯滚动条件).
        // 物理推导: L_z = I*ω + m*(r×v) 守恒, r=(0,0.5,0), I=1/6
        //   滚动条件 v_x = -0.5*ω_z  =>  v_x_final ≈ 3.0, ω_z ≈ -6.0
    ...
    fn test_mass_splitting_vs_no_splitting_stability() {
        ...
        // 4 个接触点 (模拟多点接触)
        for i in 1..=3 {
            let m = make_manifold(i, 0, Vec3::new(0.0, -0.5, 0.0), Vec3::Y, 0.1);
    ...
    fn test_multiple_contact_points() {
        ...
        // 4 个接触点 (模拟流形)
        let mut m = ContactManifold::new(1, 0);
        m.normal = Vec3::Y;
        m.compute_tangents();
    ...
    fn test_jacobi_order_independence() {
        ...
        let contacts = vec![
            (1, 0, Vec3::new(0.0, 0.0, 0.0), Vec3::Y, 0.1_f32),
