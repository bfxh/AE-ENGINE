...
//!
//! 基于 Perring-Skyrme 1962, Rubinstein 1970, Ablowitz-Kaup-Newell-Segur 1973.
...
impl SineGordonSolver {
    ...
    /// right antikink (-1) at x_R moving left (-v)
    ...
    pub fn step(&mut self) {
        ...
        for i in 0..nx {
            ...
            let new_phi = phi + d * (phi - self.phi_prev[i] + r2 * lap + dt2 * force);
...
mod tests {
    ...
    fn test_energy_conservation_kink() {
        ...
        s.initialize_kink(1.0, 0.0, 15.36);
        let e0 = s.energy();
        s.step_n(200);
        let e1 = s.energy();
    ...
    fn test_breather_periodicity() {
        ...
        let omega = 0.5_f32;
        let omega_p = (1.0 - omega * omega).sqrt();
        let period = 2.0 * std::f32::consts::PI / omega_p;
