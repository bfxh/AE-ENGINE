...
impl MetaEntity {
    ...
    pub fn apply_microstructure(
        &mut self,
        micro: &wasteland_materials::microstructure::Microstructure,
    ) {
        let props = wasteland_materials::properties::DerivedProperties::from_microstructure(micro);
    ...
    pub fn apply_quench(
        &mut self,
        temp: f32,
        medium: wasteland_materials::manufacturing::QuenchMedium,
    ) {
        let mut micro = wasteland_materials::microstructure::Microstructure::new(0.2);
        wasteland_materials::manufacturing::quench(&mut micro, temp, medium);
        self.apply_microstructure(&micro);
    }

    pub fn apply_temper(&mut self, temp: f32, duration: f32) {
        let mut micro = wasteland_materials::microstructure::Microstructure::new(0.2);
        wasteland_materials::manufacturing::quench(
            &mut micro,
            1100.0,
            wasteland_materials::manufacturing::QuenchMedium::Water,
        );
        wasteland_materials::manufacturing::temper(&mut micro, temp, duration);
        self.apply_microstructure(&micro);
    }

    pub fn apply_anneal(&mut self, temp: f32, cool_rate: f32) {
        let mut micro = wasteland_materials::microstructure::Microstructure::new(0.2);
        wasteland_materials::manufacturing::anneal(&mut micro, temp, cool_rate);
        self.apply_microstructure(&micro);
    }

    pub fn apply_cold_work(&mut self, strain: f32) {
        let mut micro = wasteland_materials::microstructure::Microstructure::new(0.2);
        wasteland_materials::manufacturing::cold_work(&mut micro, strain);
        self.apply_microstructure(&micro);
    }

    pub fn apply_forge(&mut self, temp: f32, strain: f32) {
        let mut micro = wasteland_materials::microstructure::Microstructure::new(0.2);
        wasteland_materials::manufacturing::forge(&mut micro, temp, strain);
        self.apply_microstructure(&micro);
    }

    pub fn apply_carburize(&mut self, temp: f32, duration: f32) {
        let mut micro = wasteland_materials::microstructure::Microstructure::new(0.2);
        wasteland_materials::manufacturing::carburize(&mut micro, temp, duration);
        self.apply_microstructure(&micro);
    }

    pub fn apply_age_harden(&mut self, temp: f32, duration: f32) {
        let mut micro = wasteland_materials::microstructure::Microstructure::new(0.2);
        wasteland_materials::manufacturing::age_harden(&mut micro, temp, duration);
        self.apply_microstructure(&micro);
    }
}
...
mod tests {
    ...
    fn test_quench_increases_hardness() {
        ...
        iron.apply_quench(1100.0, wasteland_materials::manufacturing::QuenchMedium::Water);
